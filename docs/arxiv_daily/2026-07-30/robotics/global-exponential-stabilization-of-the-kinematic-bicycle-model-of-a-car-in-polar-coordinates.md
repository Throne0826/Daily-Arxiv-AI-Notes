...
generated_at: "2026-07-30T07:32:04.722544+00:00"
...
定义 \(\bar\delta=delta/rho\)、\(\bar z=z/rho\)、\(\bar v=hat v/rho\)，并通过式（18）和式（19）把 \(\tilde a\)、\(\varphi\) 重写为新辅助输入 \(\hat a\)、\(\tilde\varphi\) 与状态补偿项。所得系统满足 \(\dot{bar v}=hat a\)、\(\dot{bar z}=tilde\u0007varphi-(varphi/L)bar v\)，而 \(\rho\) 和 \(\bar\delta\) 方程具有显式衰减主项及结构化耦合项。
...
- $\gamma$：视线角，满足 \(\gamma=delta-theta\)，表示目标方向与车辆航向之间的关系。
...
- $\hat v$：实际速度与期望距离比例速度 \(k_1N(k_2	u0007delta)rho\) 之间的误差。
...
极坐标把停车目标表示为 \(\rho\to0\)、\(\delta\to0\)、\(\gamma\to0\)；输入映射 \(\phi=\arctan(varphi)\) 保证任意 \(\varphi\in\mathbb R\) 都对应 \(\phi	u0007in(-\pi/2,pi/2)\)。该表示仅在 \(\rho>0\) 有效，并在 \(\rho=0\) 出现坐标奇异性。

> 直观理解：这一模块既把任务改写成“距离和角度误差归零”，也自动避免控制器命令超过模型允许的前轮转角。奇异性不是被消除，而是通过只在尚未到达目标的区域内设计闭环来处理。

**2. 非传统反步变换**

角度层采用 \(z=gamma+arctan(k_2	u0007delta)\)，速度层采用 \(\hat v=v-k_1N(k_2	u0007delta)rho\)；\(\psi_1\) 和 \(\psi_2\) 将三角差分写成在零点具有光滑延拓且全局有界的残差。该结构提取出 \(-k_1\rho\) 和 \(-k_1k_2	u0007delta\) 等稳定主项。

> 直观理解：作者不是直接抵消所有非线性，而是先安排方向和速度的理想演化，再逐层控制偏差。有界辅助函数保证角度很大或接近零时，相关三角残差仍可被统一分析。

**3. 距离归一化停车几何**

通过 \(\bar	u0007delta=delta/rho\)、\(\bar z=z/rho\)、\(\bar v=hat v/rho\) 把应随距离同步衰减的量变成新的闭环状态；因为状态空间规定 \(\rho>0\)，该变换在设计域内有定义。输入重参数化进一步把最高层误差整理为由 \(\tilde	u0007varphi\) 和 \(\hat a\) 驱动的形式。

> 直观理解：若 \(\bar	u0007delta\)、\(\bar z\)、\(\bar v\) 保持受控并趋于零，那么原角度与速度误差至少会随着距离一起缩小。这正对应车辆在最后一小段路程之前已基本对正并降速。

**训练与推理**

不适用机器学习意义上的训练或推理。在线控制时，应由测得的 \((x,y,theta,v)\) 计算目标中心极坐标，再依次计算 \(z\)、\(\hat v\) 和 \((\bar	u0007delta,ar z,ar v)\)，根据辅助反馈生成 \(\hat a\) 与 \(\tilde	u0007varphi\)，经式（18）、（19）、（13）逐层还原 \(a\) 和 \(\varphi\)，最后输出 \(a\) 与 \(\phi=arctan(varphi)\)。但所给节选没有给出 \(\hat a\) 和 \(\tilde	u0007varphi\) 的最终闭环公式，因此该在线流程的最后反馈选择原文在当前材料中未明确报告。
